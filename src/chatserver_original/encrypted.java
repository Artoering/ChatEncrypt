 ...
 */
package chatserver_original;
import Encrypted.ConvertString;
import static Encrypted.encrypt.getByteFromBits;
...
public class encrypted {
    ...
    public encrypted(  String Plaintext, String Key) {
    ...
    public String getHexaEncryption(){
        return new ConvertString().toHexa(finalEncry);
    }
    
    public void DoDecryption(String inp_encrypt,int Dynamic_PlainText) {
        finalEncry = inp_encrypt;
        finalDecry = new String();
        //JOptionPane.showMessageDialog(null, "Panjang Dynamic : "+Dynamic_PlainText);
        //DesPanel.StepsText.append("************************DECIPHER***********************************"+'\n');
        int start=0; 
        int counter=0;
        int end=64;
        for(int f=0; f<Dynamic_PlainText/8; f++){
            ...
            for(int i=0; i<8; i++){
                System.out.print((char)ch[i]); 
                if(!Character.toString((char)ch[i]).equals("*")){
                    if(finalDecry==null)
                        finalDecry=Character.toString((char)ch[i]);
                    else
                        finalDecry+=Character.toString((char)ch[i]);
...
//        System.out.println("PlainTETX: "+ PlainText.substring(start,start+9));
    ...
    private static int ch[]=new int[9999];
    ...
    private String finalDecry = new String();
